...
from util.yolov9_utils import detect_humans_yolov9
...
def process_frames_yolov9(cap, fgbg, yolo_model, display_fps=False, resize_factor=0.5, skip_frames=2, yolo_skip_frames=3):
    first_iteration_indicator = True
    first_frame = None
    accum_image = None
    frame_count = 0
    ...
    while True:
        ...
        if first_iteration_indicator:
            ...
            # Optimization 4: Optimized YOLOv9 Detection
            if yolo_frame_count % yolo_skip_frames == 0:
                human_boxes = detect_humans_yolov9(frame, yolo_model, confidence_threshold=0.25, display_boxes=True)
                yolo_frame_count = 0
            else:
                human_boxes = []  # Use the previous boxes
            ...
            for box in human_boxes:
                ...
                # Background subtraction and motion accumulation within the bounding box
                thresholded = apply_background_subtraction(fgbg, roi_frame)
                accum_image[y:h, x:w] = cv2.add(accum_image[y:h, x:w], thresholded)
